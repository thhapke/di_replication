...
##Summary 
If there is a requirement to securely replicate a table to an object store this project description and the operators could help you. The original request came from a customer who wants to replicate their ECC/S4 tables to an Azure Data Lake. For the time being, while the SLT - SAP DI connection using the planned **State Management** is not generally available (planned for Q1 2021), the replication could be done to an HANA DB and from there, picked up by the following pipeline. 

## DB to Object Store Mapping Strategy
The low storage cost of an object store faces a trade-off with lower flexibility, more precisely, you cannot update changed data records in a database as easily as csv-files. My proposal is a 2-way approach: Changes are tagged as 'I' for inserts that can be immediately appended to a CSV, while updates or deletes are tagged as 'U', change-log files are stored and periodically merged with the basic CSV file.
...
The minimum replication control data:
...
2. **Reset** After a certain pre-defined period (latency provided with the table data) the 'blocked' data is again released to be processed in the loop. The reason for not been processed might been a broken pipeline. 
...
5. **Complete** Sets the blocked package-ids to 'complete'.

Because the *HANA Client Operator* is not passing the message attributes (It receives only an sql-statement as a string at the sql-inport), the attributes have been passed over to the next operators. The operator 'Merge Attributes' is just doing what the title promises and passes to the next 'executing' operator. 

## Configuration
The configuration of the replication process is rather simple. 

### Input
The input data are the tables/views to be replicated and they are provided by a table that consists of at least two columns: 

* table name (including the schema)
* latency (seconds)

The latency determines the release on the package blocking due to the assumption that this is longer than the maximum process time for each package. 
...
* **Repl Table Dispatcher** (Python Custom Operator) 
	...
	* **Fraction of tables to parallelize** Instead of waiting until the replication of a table package has been processed, a number of packages can already be funneled into the pipeline. The maximum number is restricted by the number of tables. Fraction of '1' means that packages run in parallel to the number of tables.
...
## Parallelization
The pipeline is designed to run through a loop of all provided tables and process one packageID in each step. That means one pipeline can cover as many tables as been given by the 'tables'-table provided to the 'Table Dispatcher'-operator. The replication process can be accelerated by processing multiple table at once. This means that up to the number of tables, messages are being sent through the pipeline shortly one after the other and it does not wait until a table package has been entirely processed. This results into a 40% performance gain. 

## Graph
The graph may look complex and long at first sight but when you break it into smaller steps as described in the section "General Process", it is straightforward. 
![graph](./images/graph.png).

The 4 sql-statements have the same structure and consists of the

* **sql-statement producers** ('repl\_reset', 'repl\_block', 'repl\_select' and 'repl\_complete
...
The code of the sql-statement producer are all quite similiar and at the core of it is the generated sql-statement code: 
...
For an on-the-fly logging, all operators catch the logging-statements and send it to a log-outport. All log-outputs are collected with an **Multiplexer** and funneled to a **wiretap**-operator.
...
All operators can be directly imported from the github folder [operator solution](./solution/operators/) or the source-code [operator src](./src/di_replication), then tested and modified outside of DI. Afterwards, you can either copy and paste the script to the unmodified imported operator or use my gensolution script to create the solution. 
...
With this, the validation is quite easy, one only needs to compare the number of lines of the replicated csv-file and the sum of the "INT_NUM" columns.
...
### Package IDs
My intuition tells be that the package IDs do not need to be provided through the preceding step. There might be an SQL-statement using the order of the "DIPREPL_UPDATED" timestamp and the TOP-modifier to do this on-the-fly. I have not been successful yet because ORDER BY and UPDATE cannot easily be used in HANA SQL. An additional sub-section SELECT with a join might solve the task. 

### Table View with Shadow-Tables or SQL-JOIN Statements

Instead of creating table views when having shadow-replication tables you could also adjust the 4 sql-statements. But in this case you need to provide the primary keys for each tavle which woul make the setup far more complex than just creating table views. For this reason I decided against adding this as an option to the pipeline. 